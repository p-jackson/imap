...
public:
  ...
  bool run(std::istream& in, std::ostream& out, std::vector<std::string> args) {
    return m_runnable->run(in, out, std::move(args));
  }
...
private:
  struct Runnable {
    ...
    virtual bool run(std::istream&, std::ostream&, std::vector<std::string>) = 0;
  ...
  public:
    ...
    bool run(std::istream& in, std::ostream& out, std::vector<std::string> args) override {
      return m_innerCommand.run(in, out, move(args));
    }
